                    ...
                    <div class="overflow-x-auto">
                        ...
                        <form action="{{ route('tasks.store') }}" method="POST">
                            ...
                            <div class="mt-4">
                                ...
                                <textarea name="description" id="description" cols="10" rows="10" class="block mt-1 w-full" autocomplete="description">{{ old('description') }}</textarea>
                            ...
                            <h2 class="text-lg font-bold mb-4 mt-4">Assigned User</h2>
                            <!-- Assigned User -->
                            <div class="mt-4">
                                <x-input-label for="user_id" :value="__('Assigned User')" />
                                <select name="user_id" id="user_id" class="block mt-1 w-full">
                                    @foreach ($users as $user)
                                    <option value="{{ $user->id }}" @selected(old('user_id'==$user->id))>{{ $user->first_name }}</option>
                                    @endforeach
                                </select>
